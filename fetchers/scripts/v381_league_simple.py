"""
v3.8.1分赛事简化实验: 直接按赛事类型减draw概率
不用AH blend反推, 直接在杯赛中减draw概率(模拟低AH blend效果)
"""
...
p("=" * 70)
p("  分赛事简化实验: 杯赛减draw + 二级联赛调dt30")
p("=" * 70)

# 杯赛draw过度问题: 模型预测draw太多但precision很低
# 简化方案: 在杯赛中全局减X pp draw概率
# 二级联赛: dt30 boost=0 + base_reduce增大

for cup_dp_reduce, div2_dt30_boost, div2_dt30_base, div2_ah0_extra in [
    (0.00, 0.01, 0.01, 0.02),  # 基线v3.8.1
    (0.01, 0.01, 0.01, 0.02),  # 杯赛-1pp draw
    (0.02, 0.01, 0.01, 0.02),  # 杯赛-2pp draw
    (0.03, 0.01, 0.01, 0.02),  # 杯赛-3pp draw
    (0.02, 0.00, 0.02, 0.02),  # 杯赛-2pp + 二级dt30无boost+base0.02
    (0.02, 0.00, 0.03, 0.02),  # 杯赛-2pp + 二级dt30无boost+base0.03
    (0.03, 0.00, 0.02, 0.02),  # 杯赛-3pp + 二级0+0.02
    (0.03, 0.00, 0.03, 0.02),  # 杯赛-3pp + 二级0+0.03
]:
    ...
    for m in matches:
        ...
        if is_cup and cup_dp_reduce > 0:
            ...
            nd = hp_x + ap_x
            if nd > 0:
                hp_x += cup_dp_reduce * (hp_x / (hp_x + ap_x))
                ap_x += cup_dp_reduce * (ap_x / (hp_x + ap_x))

        # dt处理
        is_dt30 = 'draw_threshold_0.3' in v34_flags
        if is_dt30:
            if is_div2:
                boost = div2_dt30_boost; base = div2_dt30_base; ah0_e = div2_ah0_extra
            else:
    ...
    p(f"\n  cup_reduce={cup_dp_reduce:.2f} div2=({div2_dt30_boost},{div2_dt30_base},{div2_ah0_extra}):")
    ...
    if cup_n > 0:
        ...
        dr = cup_draw_correct/cup_draw_actual*100 if cup_draw_actual > 0 else 0
        p(f"    杯赛: {cup_correct}/{cup_n}={cup_correct/cup_n*100:.1f}% draw={cup_draw_pred} prec={dp:.0f}%")
...
OUTPUT = 'd:/football_tools/fetchers/scripts/v381_league_simple.txt'
