...
今日体彩分析 (5001-5009)
"""
import sys, io, json, requests, math
...
r = session.get(url, params={'sellStatus': 'on', 'date': '2026-05-29'}, timeout=30)
data = r.json()

# 只取5001-5009
matches = []
for dg in data.get('value', {}).get('matchInfoList', []):
    bd = dg.get('businessDate', '')
    if bd != '2026-05-29': continue
    for m in dg.get('subMatchList', []):
        raw_num = m.get('matchNum', '')
        num = str(raw_num) if raw_num else ''
        if not num.startswith('5'): continue
        had = m.get('had', {})
        hhad = m.get('hhad', {})
        crs = m.get('crs', {})
        ttg = m.get('ttg', {})
...
def predict(oh, od, oa, league=''):
    ...
    is_cup = any(kw in league for kw in ['欧冠','欧联','国际赛','友谊赛'])
    if is_cup:
        dp -= 0.02; nd = hp+ap
    ...
    return {'hp':hp,'dp':dp,'ap':ap,'pred':pred,'conf':conf,'margin':margin,'ev_h':ev_h,'ev_d':ev_d,'ev_a':ev_a}
...
for rec in all_recs:
    m = rec[0]
    dir, ev, odds, prob = rec[1]
    marker = ' ★' if ev > -0.05 else ''
    match_str = m['home'] + ' vs ' + m['away']
    p(f'  {m["num_str"]:6s} {m["league"]:6s} {match_str:28s} {dir:6s} {ev:+.3f}  {odds:.2f}  {prob*100:.1f}%{marker}')
