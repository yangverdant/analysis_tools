"""
v3.8.1完整增量验证: 分档dp reduce
从v3.4存储结果出发, 逐步应用v3.8.1的所有变更
"""
...
def main():
    ...
    p("=" * 70)
    p("  v3.8.1完整增量验证: 分档dp reduce + 全部v3.5+规则")
    p("=" * 70)

    total_n = 0; correct = 0; brier = 0.0; net_gain = 0
    subgroups = defaultdict(lambda: {'n': 0, 'correct': 0, 'brier': 0.0})
    ...
    for m in matches:
        ...
        if is_dt30:
            ...
            non_draw = hp_x + ap_x
            if non_draw > 0:
                hp_x -= DT_V38[0.30] * (hp_x / non_draw)
                ap_x -= DT_V38[0.30] * (ap_x / non_draw)
            hp_x, dp_x, ap_x = normalize_probs(hp_x, dp_x, ap_x)
            # 分档reduce
            dt30_ah_type = get_ah_type(conn, mk)
            if dt30_ah_type == 'ah0':
                reduce_val = DT30_BASE_DP_REDUCE + DT30_AH0_EXTRA_DP_REDUCE
            else:
                reduce_val = DT30_BASE_DP_REDUCE
            if reduce_val > 0:
                dp_x -= reduce_val
                non_draw = hp_x + ap_x
                if non_draw > 0:
            ...
            non_draw = hp_x + ap_x
            if non_draw > 0:
                hp_x -= DT_V38[0.28] * (hp_x / non_draw)
                ap_x -= DT_V38[0.28] * (ap_x / non_draw)
            hp_x, dp_x, ap_x = normalize_probs(hp_x, dp_x, ap_x)
        elif is_dt26:
            dp_x += DT_V38[0.26]
            non_draw = hp_x + ap_x
            if non_draw > 0:
                hp_x -= DT_V38[0.26] * (hp_x / non_draw)
                ap_x -= DT_V38[0.26] * (ap_x / non_draw)
            hp_x, dp_x, ap_x = normalize_probs(hp_x, dp_x, ap_x)
        ...
        if is_dt30:
            ...
            if actual == 'home':   subgroups[sg_key]['brier'] += (hp_x-1)**2 + dp_x**2 + ap_x**2
            elif actual == 'draw': subgroups[sg_key]['brier'] += hp_x**2 + (dp_x-1)**2 + ap_x**2
            else:                  subgroups[sg_key]['brier'] += hp_x**2 + dp_x**2 + (ap_x-1)**2
    ...
    p(f"\n  === v3.8.1 (分档dp reduce: base={DT30_BASE_DP_REDUCE}, ah0_extra={DT30_AH0_EXTRA_DP_REDUCE}) ===")
    p(f"  总体: argmax={correct}/{total_n}={correct/total_n*100:.2f}% Brier={brier/total_n:.4f} net={net_gain:+d}")
    p(f"  draw: pred={draw_pred_n} prec={draw_pred_correct/draw_pred_n*100:.1f}% actual={draw_actual} recall={draw_pred_correct/draw_actual*100:.1f}%")

    for key, g in sorted(subgroups.items()):
        if g['n'] == 0: continue
        acc = g['correct'] / g['n'] * 100
        br = g['brier'] / g['n'] if 'brier' in dir(g) else 0
        p(f"  [{key}] {g['correct']}/{g['n']}={acc:.1f}% Brier={br:.4f}")

    p(f"\n{'=' * 70}")
    p("  验证完成")
    p("=" * 70)

    OUTPUT = 'd:/football_tools/fetchers/scripts/v381_validation.txt'
