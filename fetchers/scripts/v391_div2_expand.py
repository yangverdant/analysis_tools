"""
v3.9.1扩展: 所有二级联赛和高draw联赛分别测试全局减draw
serie_b全局减0.02 draw → +2pp, 看其他联赛是否也受益
"""
...
p("=" * 80)
p("  v3.9.1扩展: 全局减draw vs dt30调整, 按联赛对比")
p("=" * 80)
...
# 策略: 对所有div2联赛, 用"全局减0.02draw"替代dt30调整
for strategy in ['v39_div2_dt30', 'div2_global_reduce', 'div2_global_reduce_001']:
    total_n = 0; correct = 0; brier = 0.0; net_gain = 0
    cup_n = 0; cup_correct = 0; cup_draw_pred = 0
    top5_n = 0; top5_correct = 0

    league_acc = defaultdict(lambda: {'n': 0, 'correct': 0, 'draw_pred': 0, 'draw_actual': 0, 'draw_correct': 0})
    ...
    for m in matches:
        ...
        if is_dt30:
            if is_div2:
                if strategy == 'v39_div2_dt30':
                    # v3.9: boost=0, base=0.02, ah0=0.02
                    boost = 0.00; base = 0.02; ah0_e = 0.02
                elif strategy == 'div2_global_reduce':
                    # 全局减0.02 draw (替代dt30)
                    dp_x -= 0.02; nd = hp_x+ap_x
                    if nd > 0: hp_x += 0.02*(hp_x/nd); ap_x += 0.02*(ap_x/nd)
                    hp_x, dp_x, ap_x = normalize_probs(hp_x, dp_x, ap_x)
                    boost = 0.00; base = 0.00; ah0_e = 0.00
                elif strategy == 'div2_global_reduce_001':
                    # 全局减0.01 draw
        ...
        if is_cup: cup_n += 1; cup_correct += (1 if pred_x == actual else 0); cup_draw_pred += (1 if pred_x == 'draw' else 0)
        if is_top5: top5_n += 1; top5_correct += (1 if pred_x == actual else 0)

        # 按联赛统计
        for lk, lk_name in LEAGUES_TO_TRACK.items():
            if lk in league.lower():
                la = league_acc[lk_name]
                la['n'] += 1
                la['correct'] += (1 if pred_x == actual else 0)
                if pred_x == 'draw': la['draw_pred'] += 1
                if actual == 'draw': la['draw_actual'] += 1
                if pred_x == 'draw' and actual == 'draw': la['draw_correct'] += 1
                break
    ...
    p(f"    --- 逐联赛 ---")
    for lk_name in ['serie_b', 'la_liga_2', 'ligue_2', '2_bundesliga', 'segunda', 'efl_championship',
                    'primeira_liga', 'premier_league', 'la_liga', 'serie_a', 'bundesliga', 'ligue_1']:
        la = league_acc.get(lk_name)
        if la and la['n'] > 0:
            acc = la['correct']/la['n']*100
            draw_prec = la['draw_correct']/la['draw_pred']*100 if la['draw_pred'] > 0 else 0
            p(f"      {lk_name:20s} {la['correct']:3d}/{la['n']:3d}={acc:5.1f}% draw_pred={la['draw_pred']:3d} prec={draw_prec:.0f}% actual_draw={la['draw_actual']}")

p(f"\n{'=' * 80}")
p("  扩展验证完成")
p("=" * 80)

OUTPUT = 'd:/football_tools/fetchers/scripts/v391_div2_expand.txt'
