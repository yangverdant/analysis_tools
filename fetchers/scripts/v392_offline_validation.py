"""
v3.9.2离线验证: v3.9.1 + 赛季末均衡draw回调
cup=0.02 + prima=0.02 + div2_dt30(0,0.02,0.02) + late_season_balanced_draw(0.01)
"""
...
# v3.9.2新增参数
LATE_SEASON_DRAW_BOOST = 0.01
LATE_SEASON_BALANCED_THRESHOLD = 3.0
LATE_SEASON_DRAW_THRESHOLD = 0.28
LATE_SEASON_MONTHS = [4, 5, 6]
...
def get_odds(conn, mk):
    euro_row = conn.execute(
        "SELECT data_json FROM match_data WHERE match_key=? AND source='factor' AND data_type='factor:euro_odds'",
        (mk,)).fetchone()
    if not euro_row: return None, None, None
    raw = json.loads(euro_row['data_json']).get('raw', {})
    oh = float(raw.get('avg_home_odds', 0) or raw.get('closing_avg_home_odds', 0) or 0)
    od = float(raw.get('avg_draw_odds', 0) or raw.get('closing_avg_draw_odds', 0) or 0)
    oa = float(raw.get('avg_away_odds', 0) or raw.get('closing_avg_away_odds', 0) or 0)
    return oh, od, oa
...
p("=" * 70)
p("  v3.9.2离线验证: v3.9.1 + late_season_balanced_draw(0.01)")
p("=" * 70)
...
lsb_n = 0; lsb_correct = 0  # late_season_balanced触发场次
lsb_draw_pred = 0
...
for m in matches:
    ...
    oh, od, oa = get_odds(conn, mk)
    if oh and 1.25 <= oh <= 1.35 and match_month in [5, 6]:
        hp_x -= 0.02; dp_x += 0.01; ap_x += 0.01
        hp_x, dp_x, ap_x = normalize_probs(hp_x, dp_x, ap_x)

    # 南美杯赛冷门区
    is_sa = any(kw in league for kw in SA_KEYWORDS)
    if is_sa and oh and 1.35 <= oh <= 1.50:
        hp_x -= 0.02; dp_x += 0.01; ap_x += 0.01
        hp_x, dp_x, ap_x = normalize_probs(hp_x, dp_x, ap_x)

    # ★ v3.9.2新增: 赛季末均衡draw回调
    trigger_lsb = False
    if oh and oa and od and match_month in LATE_SEASON_MONTHS:
        is_balanced = oh < LATE_SEASON_BALANCED_THRESHOLD and oa < LATE_SEASON_BALANCED_THRESHOLD
        implied_draw = 1.0/od if od > 1 else 0
        is_high_draw = implied_draw >= LATE_SEASON_DRAW_THRESHOLD
        if is_balanced and is_high_draw:
            dp_x += LATE_SEASON_DRAW_BOOST
            nd = hp_x + ap_x
            if nd > 0:
                hp_x -= LATE_SEASON_DRAW_BOOST * (hp_x/nd)
                ap_x -= LATE_SEASON_DRAW_BOOST * (ap_x/nd)
            hp_x, dp_x, ap_x = normalize_probs(hp_x, dp_x, ap_x)
            trigger_lsb = True
    ...
    if trigger_lsb:
        lsb_n += 1
        if pred == actual: lsb_correct += 1
        if pred == 'draw': lsb_draw_pred += 1
...
if lsb_n > 0: p(f"  赛季末均衡触发: {lsb_correct}/{lsb_n}={lsb_correct/lsb_n*100:.1f}% draw_pred={lsb_draw_pred}")

p(f"\n  v3.9.1基线: 50.55% Brier=0.6033 net=+124")
delta = correct/total_n*100 - 50.55
p(f"  Δ: {delta:+.2f}pp")
...
OUTPUT = 'd:/football_tools/fetchers/scripts/v392_offline_validation.txt'
