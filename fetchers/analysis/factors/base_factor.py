...
class BaseFactor(ABC):
    ...
    def _numeric(self, home_value, away_value, unit: str = "",
        ...
        """构建数值型因素"""
        # 允许子类传入预计算的diff（如elo_rating归一化后的值）
        precomputed_diff = raw.pop("diff", None)
    ...
    def _get_standings_map(self, league: str, storage: UnifiedStorage,
                           season: str = None) -> Dict[str, Dict]:
        standings = storage.get_standings(league, season=season)
        if not standings:
            # fallback: no season filter
    ...
    def _get_season_from_date(self, date_str: str) -> str:
        """从比赛日期推算赛季（跨年赛季如2024-2025→2024）"""
        if not date_str:
            return ""
        year = int(date_str[:4])
        month = int(date_str[5:7]) if len(date_str) >= 7 else 1
        # 8月之后属于新赛季起始年份，8月之前属于上赛季
        if month >= 8:
            return str(year)
        else:
            return str(year - 1)
