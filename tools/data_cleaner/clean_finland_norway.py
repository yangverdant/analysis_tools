...
DATA_DIR = Path("D:/football_tools/data/01_leagues")
OUTPUT_DIR = Path("D:/football_tools/new_data/leagues")

LEAGUES = {
    'finland': ('veikkausliiga', 'Veikkausliiga', 6),  # 12队, 每轮6场
    'norway': ('eliteserien', 'Eliteserien', 8),  # 16队, 每轮8场
}
...
def clean_file(input_path, output_path, season, matches_per_round):
    rows = []
    try:
...
if __name__ == '__main__':
    ...
    for folder, (league_name, div_name, matches_per_round) in LEAGUES.items():
        print(f'\n{league_name}:')
        data_path = DATA_DIR / folder
        output_path = OUTPUT_DIR / league_name

        csv_files = sorted(data_path.glob(f'{league_name}_*.csv'))
        total = 0

        for csv_file in csv_files:
            if csv_file.name == f'{league_name}_raw.csv':
                continue

            year = csv_file.stem.replace(f'{league_name}_', '')
            if year.isdigit():
                year_int = int(year)
            ...
            count = clean_file(csv_file, output_file, season, matches_per_round)
            if count > 0:
