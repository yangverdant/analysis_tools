...
DFB-Pokal API ID: 178
DFL-Supercup API ID: 176
"""
...
def update_cup(api_id: int, name: str, db_id: int, from_date: str, to_date: str):
    """更新杯赛数据"""
...
def show_stats():
    """显示统计"""
    print("\n" + "=" * 60)
    print("German Cup Competitions - Updated Data")
    print("=" * 60)

    conn = get_db()
    cursor = conn.cursor()

    for db_id, name in [(7482, 'DFB-Pokal'), (7483, 'DFL-Supercup')]:
        cursor.execute('''
        ...
        cursor.execute('SELECT MIN(match_date), MAX(match_date), COUNT(*) FROM matches WHERE league_id = ?', (db_id,))
        r = cursor.fetchone()
        print(f"  Total: {r[2]} matches ({r[0]} ~ {r[1]})")
...
if __name__ == "__main__":
    print("=" * 60)
    print("Updating German Cup Competitions")
    print("=" * 60)

    # 更新2020-2026
    for api_id, (name, db_id) in CUPS.items():
        update_cup(api_id, name, db_id, "2020-08-01", "2026-12-31")
