...
# 需要补充的赛事
COMPETITIONS = {
    4: ('UEFA Europa League', 7511),  # 欧联杯
    683: ('UEFA Conference League', 7512),  # 欧协联
}
...
def update_competition(api_id: int, name: str, db_id: int, from_date: str, to_date: str):
    """从API更新数据"""
    print(f"\nUpdating {name} ({from_date} to {to_date})...")
...
def show_stats():
    ...
    for db_id, name in [(7511, 'UEFA Europa League'), (7512, 'UEFA Conference League')]:
        ...
        # Count by season
        cursor.execute('''
            SELECT substr(match_date, 1, 4) as year, COUNT(*)
...
if __name__ == "__main__":
    print("=" * 60)
    print("Supplementing UEFA Competition Data (2023-2026)")
    print("=" * 60)

    # Update each competition for 2023-2026
    for api_id, (name, db_id) in COMPETITIONS.items():
        # Split into smaller date ranges to avoid timeout
        update_competition(api_id, name, db_id, "2023-02-01", "2023-12-31")
        update_competition(api_id, name, db_id, "2024-01-01", "2024-12-31")
        update_competition(api_id, name, db_id, "2025-01-01", "2025-12-31")
        update_competition(api_id, name, db_id, "2026-01-01", "2026-12-31")
